...
from llm_extractor_pipeline import build_messages, llm_ensemble_extractor, llm_extractor
...
PERFORMANCE_STORE_PATH = BASE_DIR / "pipeline_performances.json"
...
def load_saved_performances() -> dict:
    """Load saved pipeline evaluation performances from disk."""
    if not PERFORMANCE_STORE_PATH.exists():
        return {}
    try:
        return json.loads(PERFORMANCE_STORE_PATH.read_text(encoding="utf-8"))
    except Exception:
        return {}


def save_pipeline_performance(pipeline_id: int, payload: dict) -> None:
    """Persist a performance summary for a pipeline."""
    store = load_saved_performances()
    store[str(pipeline_id)] = payload
    PERFORMANCE_STORE_PATH.write_text(json.dumps(store, indent=2), encoding="utf-8")


def delete_pipeline_performance(pipeline_id: int) -> None:
    """Remove a saved performance summary for a pipeline."""
    store = load_saved_performances()
    if str(pipeline_id) in store:
        store.pop(str(pipeline_id))
        PERFORMANCE_STORE_PATH.write_text(json.dumps(store, indent=2), encoding="utf-8")
    ...
    if (state.selec_row is not None) :
        ...
        with extract_cols[0] :
            if st.button("Run Extraction") :
                ...
                anti = pipeline.get("anti_hallucination", {})
                ensemble_flag = bool(anti.get("ensemble", anti.get("ensembleSize", False)))
                ensemble_size = int(anti.get("ensemble_size", anti.get("ensembleSize", 2)) or 2)
                state.model_config = []
                ...
                ###Run the extraction
                if ensemble_flag and len(state.model_config) >= 2:
                    data_extract = llm_ensemble_extractor(
                        state.prompt,
                        state.filt_data,
                        state.model_config,
                        ensemble_size=min(ensemble_size, len(state.model_config)),
                    )
                else:
                ...
                notice_msg = ""
                if "ensemble_notice" in state.data_extract.columns:
                    try:
                        notice_msg = state.data_extract["ensemble_notice"][0]
                    except Exception:
                        notice_msg = ""
                if notice_msg:
                ...
                if isinstance(state.data_extract, pl.DataFrame):
                    concept_payload = state.data_extract["concept_extracted"][0]
                    entries = None
                    if isinstance(concept_payload, pl.Series):
                        entries = concept_payload.to_list()
                    elif isinstance(concept_payload, list):
                        entries = concept_payload
                        
                    if entries:
                        state.json_str = entries
                        state.extractor_bool = True
                        result = pl.DataFrame(entries)
                        st.dataframe(result, use_container_width=True)
                    else:
                        st.warning("Format de sortie pas respectee")
                else :
                    st.warning("sortie dans le mauvais format")

            else:
                st.warning("Utilisez le bouton pour afficher les resultats de l'extraction ici.")

            with extract_cols[1]:
                if "json_str" in state and state.extractor_bool:
                    display(highlight_contexts(state.text_brut, state.json_str, state.ratio_treshold),False)
                # else :
                #     st.warning("Aucune tentative d'extraction")
            
    else :
        st.warning("Selectionner une ligne avant de lancer l'extraction")
...
def render_gold_standard_tab(id_col: str | None, concept_cols: list[str]) -> None:
    ...
    if result_payload:
        ...
        saved_perf = load_saved_performances().get(str(result_payload.get("pipeline_id")))
        action_cols = st.columns(2)
        if action_cols[0].button("Save performance", key=f"save_perf_{result_payload.get('pipeline_id')}", disabled=result_payload.get("pipeline_id") is None):
            payload_to_save = {
                "pipeline_id": result_payload.get("pipeline_id"),
                "pipeline_name": result_payload.get("pipeline_name"),
                "timestamp": result_payload.get("timestamp"),
                "summary": summary,
            }
            try:
                save_pipeline_performance(result_payload.get("pipeline_id"), payload_to_save)
                st.success("Performance saved.")
                saved_perf = payload_to_save
            except Exception as exc:
                st.error(f"Unable to save performance: {exc}")

        if saved_perf and action_cols[1].button("Delete saved performance", key=f"delete_perf_{result_payload.get('pipeline_id')}"):
            try:
                delete_pipeline_performance(result_payload.get("pipeline_id"))
                st.info("Saved performance deleted.")
                saved_perf = None
            except Exception as exc:
                st.error(f"Unable to delete performance: {exc}")

        if saved_perf:
            st.info(
                f"Saved performance for {saved_perf.get('pipeline_name', 'pipeline')} ("
                f"{saved_perf.get('timestamp', 'n/a')})."
            )
