...
| `annotator*_medical_concepts.csv` | Sample gold-standard style labels (Diabète, HTA, IC, BPCO) generated by the script above. |
...
> ℹ️ `llm_extractor_pipeline.py` currently points to a specific Ollama base URL inside `load_model_ollama`. Update the `base_url` (or read it from `OLLAMA_BASE_URL`) if you are hosting the models elsewhere.
